...
def parse_arg():
    parser = argparse.ArgumentParser(
        'Extract face images that have specific features from CelebA dataset')
    ...
    parser.add_argument('source_list', type=str,
                        help='Output directory for images without specific feature')
    parser.add_argument('target_list', type=str,
                        help='Output directory for images with specific feature')
    parser.add_argument('feature', type=str,
                        help='Attribute(s) to separate output images')
    parser.add_argument('include', type=str,
                        help='Attribute(s) output images should have')
    parser.add_argument('--exclude', '-e', type=str, default='',
                        help='Attribute(s) output images should not have')
...
def main():
    ...
    attributes = np.loadtxt(args.attr_file, skiprows=2,
                            usecols=cols, dtype=np.int32)
    source_paths = []
    target_paths = []
